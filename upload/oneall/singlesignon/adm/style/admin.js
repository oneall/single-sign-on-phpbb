$(function() {
    $(document).ready(function($) {
        ...
        $('#oa_sso_autodetect_api_connection_handler').click(function() {
            ...
            if ($(button).hasClass('working') === false) {
                ...
                message_container = $('#oa_sso_api_connection_handler_result');
                message_container.removeClass('success_message error_message').addClass('working_message');
                message_container.html('');

                $.post(OA_SINGLE_SIGN_ON_AJAX_URL_AUTODETECT, data, function(response) {

                    /* CURL/FSOCKOPEN Radio Box */
                    var radio_curl = $("#oa_sso_api_connection_handler_curl");
                    var radio_fsockopen = $("#oa_sso_api_connection_handler_fsockopen");
                    var radio_port_80 = $("#oa_sso_api_connection_port_80");
                    var radio_port_443 = $("#oa_sso_api_connection_port_443");
        ...
        /* Verify API Settings */
        $('#oa_sso_test_api_settings').click(function() {
            var button = this;
            if ($(button).hasClass('working') === false) {
                $(button).addClass('working');
                ...
                var radio_fsockopen_val = $("#oa_sso_api_connection_handler_fsockopen:checked").val();
                var radio_port_443 = $("#oa_sso_api_connection_port_443:checked").val();

                var subdomain = $('#oa_sso_api_subdomain').val();
                var key = $('#oa_sso_api_key').val();
                var secret = $('#oa_sso_api_secret').val();
                ...
                message_container = $('#oa_sso_api_test_result');
                message_container.removeClass('success_message error_message').addClass('working_message');
                message_container.html('');

                $.post(OA_SINGLE_SIGN_ON_AJAX_URL_VERIFY, data, function(response) {
                    message_container.removeClass('working_message');
                    message_container.html(response.message);
